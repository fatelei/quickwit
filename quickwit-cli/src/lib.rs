...
/// File URIs are resolved (normalised) to absolute path.
/// Handles special path characters like (~, ., ..)
pub fn normalize_uri(uri: &str) -> anyhow::Result<String> {
    let parts: Vec<&str> = uri.split("://").collect();
    let (protocol, mut path) = match parts.len() {
        1 => (FILE_PROTOCOL, String::from(parts[0])),
        _ => (
            parts[0],
            parts
                .into_iter()
    ...
    if protocol == FILE_PROTOCOL {
        if path.starts_with('~') {
            let home_dir_path = home::home_dir()
                .context("Could nto get home directory.")?
                .to_string_lossy()
                .to_string();
...
/// Normalizes a path without checking file existence in contrary to `Path::canonicalize`.
...
mod tests {
    ...
    fn test_normalize_uri() -> anyhow::Result<()> {
        ...
        assert_eq!(
            normalize_uri("home/hommer/docs/../dognuts")?,
            String::from("file:///home/hommer/dognuts")
        );

        assert_eq!(
            normalize_uri("file://home/hommer/docs/dognuts")?,
            String::from("file:///home/hommer/docs/dognuts")
        );

        assert_eq!(
            normalize_uri("~/")?,
            format!("file://{}", home::home_dir().unwrap().to_str().unwrap())
        );

        assert!(normalize_uri("~")?.starts_with("file:///"));
        assert!(normalize_uri("~/")?.starts_with("file:///"));
        assert!(normalize_uri("~/.")?.starts_with("file:///"));
        assert!(normalize_uri("~/..")?.starts_with("file:///"));
        assert!(normalize_uri("~/../../..")?.starts_with("file:///"));

        assert_eq!(normalize_uri("file://")?, String::from("file:///"));
